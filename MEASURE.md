...
## pprof結果 (bench中30s CPU断面): テンプレ描画が支配
- text/template.evalCommand 36.8%, evalCall 27.9%, reflect.Value.call 23.6%, mallocgc 12.8%
- 結論: GET/ の時間は html/template のリフレクション描画。DB/memcacheではない
- 400kの本丸=③ テンプレ描画削減。方針: post単位レンダリング断片キャッシュ(CSRFはプレースホルダ差替) or ホットパスの手書きHTML化

## bench #7-9: ③断片キャッシュ+socketで退行 (271k->160k)
- #7=160375 #8=158610 (fail0), pprof中#9=129495(fail3:403→200/422→200/404)
- pprof: CPU使用率39%(↓ from 56%)=待ち直列化。template evalCall12%/reflect10%残存=断片キャッシュ非効率。futex2.5%(重い競合ではない)
- 判断: ③(f40a028)をrevert→rebuild→再ベンチで原因切り分け(③ or socket)。271k回復なら③設計やり直し

## 提出結果 (公式ベンチ, #5構成 394b7e6)
- score 217512 / pass:true / success 210054 / fail 2
- fail: GET /@lucy timeout, GET /admin/banned timeout（未最適化の重経路）
- baseline 565 比 約385倍
